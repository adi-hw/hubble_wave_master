...
async function bootstrap() {
  ...
  const port =
    process.env.PORT ||
    process.env.INSIGHTS_PORT ||
    process.env.PORT_INSIGHTS ||
    process.env.PORT_SVC_INSIGHTS ||
    3007;

  await app.listen(port);
  Logger.log(`Insights service running on: http://localhost:${port}/${globalPrefix}`);
}
