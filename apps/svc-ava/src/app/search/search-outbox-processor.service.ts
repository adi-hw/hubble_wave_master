...
import { RecordEventPayload } from './search.types';

@Injectable()
export class SearchOutboxProcessorService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(SearchOutboxProcessorService.name);
  ...
  constructor(
    ...
    this.batchSize = parseInt(this.configService.get('SEARCH_OUTBOX_BATCH_SIZE', '20'), 10);
    this.pollIntervalMs = parseInt(this.configService.get('SEARCH_OUTBOX_POLL_MS', '2000'), 10);
    this.lockTimeoutMs = parseInt(this.configService.get('SEARCH_OUTBOX_LOCK_TIMEOUT_MS', '60000'), 10);
  }
  ...
  private async poll(): Promise<void> {
    ...
    try {
      ...
      for (const entry of entries) {
        try {
          ...
          const recordPayload: RecordEventPayload = {
            eventType:
              typeof payload.eventType === 'string' ? payload.eventType : entry.eventType,
            collectionCode: entry.collectionCode || (payload.collectionCode as string) || '',
          ...
          if (!recordPayload.collectionCode || !recordPayload.recordId) {
            throw new Error('Search outbox record event missing collectionCode or recordId');
          }

          await this.indexingService.handleRecordEvent(recordPayload);
      ...
      this.logger.error(`Search outbox poll failed: ${(error as Error).message}`);
    } finally {
