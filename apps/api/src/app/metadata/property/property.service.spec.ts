...
function makeQb(): QueryBuilderStub {
  const qb: QueryBuilderStub = {
    ...
    getRawMany: jest.fn().mockResolvedValue([{ max: 0 }]),
  };
...
describe('PropertyService', () => {
  ...
  let collectionRepo: { findOne: jest.Mock; find: jest.Mock };
  let propertyTypeRepo: { findOne: jest.Mock };
  let referenceScanner: { findReferences: jest.Mock };
  let qb: QueryBuilderStub;
  let revisionQb: QueryBuilderStub;
  ...
  beforeEach(async () => {
    ...
    revisionRepo = {
      createQueryBuilder: jest.fn(() => revisionQb),
      create: jest.fn((data) => ({ id: 'rev-1', ...data })),
    ...
    collectionRepo = {
      findOne: jest.fn().mockResolvedValue({ id: 'col-1', applicationId: 'app-1' }),
      find: jest.fn().mockResolvedValue([]),
    };
    propertyTypeRepo = {
      findOne: jest.fn().mockResolvedValue({ id: 'type-1' }),
    };
    referenceScanner = {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(PropertyDefinitionRevision), useValue: revisionRepo },
  ...
  describe('createProperty — column-name collision (W2.C)', () => {
    ...
    it('rejects bulk-create requests with in-request column-name collisions', async () => {
      qb.getOne.mockResolvedValue(null);
      propertyRepo.find.mockResolvedValue([]);

      // Two distinct codes that truncate to the same 63-char column name.
      ...
      ).rejects.toThrow(/duplicate columnNames in request/);
    });
  });
