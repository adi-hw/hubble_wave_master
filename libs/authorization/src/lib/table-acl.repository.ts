...
import { TableAcl, Permission } from '@eam-platform/platform-db';
import { In, IsNull } from 'typeorm';

@Injectable()
export class TableAclRepository {
  constructor(private readonly tenantDb: TenantDbService) {}

  async findByTableAndOperation(tenantId: string, tableName: string, operation: string) {
    const repo = await this.tenantDb.getRepository<TableAcl>(tenantId, TableAcl as any);
    return repo.find({
      where: [
