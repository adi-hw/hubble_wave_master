...
export class TenantContextService {
   ...
   */
  async getTenantProfile(dataSource: DataSource, tenantId: string): Promise<TenantProfile> {
    // Check cache
    ...
    // Build fresh profile
    const profile = await this.buildTenantProfile(dataSource, tenantId);

    // Cache it
    this.profileCache.set(tenantId, { profile, cachedAt: new Date() });
