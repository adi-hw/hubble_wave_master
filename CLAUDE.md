...
- 2026-05-13 (Plan Fix 40): end-to-end bootstrap hardening sweep.
  ...
  generated by `scripts/setup.ts` + added to `REQUIRED_ENV_VARS` +
  `ConnectorCredentialsService` constructor no longer throws at module
  load (region check deferred to first `resolveCredentials()` call),
  (2) `GridModule` imports `CacheModule.register()` so its own
  `ModelRegistryService` provider can resolve `CACHE_MANAGER`,
  (3) `AutomationRuntimeModule` now exports `ActionHandlerService` and
  `ScriptSandboxService` so `SchedulerService` in `SchedulingModule`
  can resolve its constructor dependencies. Proactive static audit of
  `@Injectable` constructors caught 2 additional latent wirings:
  `IntegrationModule` and `WorkflowModule` both host services that
  inject `EventEmitter2` — both now import `EventEmitterModule.forRoot()`
  explicitly instead of relying on `MetadataModule`'s earlier `forRoot()`
  call propagating globally. Bootstrap testing established the cumulative
  cost of "ships compile-but-doesn't-boot": a follow-up CI step that
  runs `nx serve api` (fail on DI errors, pass on DB-connect errors) is
  recommended to catch wiring issues at PR time.
...
- 2026-05-13 (Plan Fix 32): cleanup PR — W5.J false-positive entry
  ...
  marked Complete with summary of W5.A/B/G/I outcomes. CLAUDE.md §24
  reorder: Plan Fix 31 moved above Plan Fix 30 PR-2 to restore commit-
  landing chronological order (the post-hoc rebase in PR #48 had
  swapped them).
