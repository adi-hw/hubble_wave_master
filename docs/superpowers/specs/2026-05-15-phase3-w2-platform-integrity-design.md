...
- Service-token data-access test (separate concern from scope): svc-worker presents a token with `automation:invoke` scope and hits the data-ACL route `GET /api/data/collections/:collectionCode/data` (the runtime path — `CollectionDataController` declares `@Controller('data/collections')` and `main.ts:15` applies the global `api` prefix). Access is determined by the service principal's `CollectionAccessRule` grants via the §28 evaluator, not by the JWT scope claim. Scope says "can call automation"; ACL says "can read this collection." Both layers apply independently.
