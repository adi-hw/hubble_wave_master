...
| **G0b** Advanced | 11 | §13.3 + §13.7 (Merkle) + §13.8 + §13.9 + §13.10 + §13.11 + §13.12 + §13.20 (PRs B1-B6 §3.19 customization core+5 adapters) + §13.21 (PRs D1-D3 §3.20 rounds core) + §13.22 (PRs E1-E3 §3.21 reporting core) | ~64 | task_projection circuit breaker + Merkle batch + mobile parity + kiosk + customization override core + rounds core + reporting core + ReportBuilder |
| **G0c** Marquee substrate | 17 | §13.13-§13.19 + §13.20 (PRs C1-C7 remaining customization adapters + classifier + scanner) + §13.21 (PRs D4-D7 §3.20 rounds completion) + §13.22 (PRs E4-E10 §3.21 reporting completion) | ~56 | Storage + Connector + Vector + Graph + Financial + Import + Secrets + ALL §3.19 adapters + ALL §3.20 rounds primitives + ALL §3.21 reporting framework primitives live |
...
**Total: ~280 PRs across 50 weeks (~250 original + 13 §3.19 customization override + 7 §3.20 rounds + 10 §3.21 reporting framework — all added 2026-05-18 per founder direction). Critical-path PR throughput required: ~6 PRs/week sustained, with parallelism on independent tracks. G0c slip risk elevated: 56 PRs in 6 weeks is the tightest gate in the schedule.**
...
| **D1** | §13.21 PR-1 | Schema + 3 rounds tables + entities + `RoundsDefinitionService` CRUD + admin endpoint + permission codes | 016, 020 |
| **D2** | §13.21 PR-2 | `RoundsSessionService.createAdHoc/createFromTemplate/recordStopOutcome/completeSession/abandonSession` + auto rule 20 + §3.5 + §3.6 integration | D1 |
| **D3** | §13.21 PR-3 | `RoundsGenerationProcessor` worker + idempotency on (round_definition_id, scheduled_at) + §3.3 SchedulingModule hook | D1, 006 |
| **E1** | §13.22 PR-1 | Schema + 5 analytics tables + entities + `ReportDefinitionService` CRUD + 5 permission codes | — |
| **E2** | §13.22 PR-2 | `<ReportBuilder>` plugin: drag-source/group/aggregate + filter builder + preview | E1, 042-044 |
| **E3** | §13.22 PR-3 | `ReportMaterializationProcessor` real_time path + snapshot_* policies + cardinality heuristic + matview-vs-table selector | E1 |
...
| **D4** | §13.21 PR-4 | `<RoundsRunner>` Glove-Mode swipe-deck mobile + desktop adapter via §3.7 parity + `<StopBuilder>` ad-hoc primitive | D2, 042-044 |
| **D5** | §13.21 PR-5 | Cross-pack target resolution via §3.15 graph + canon §28 evaluator pass-through + canon §46 amendment | D2, 096 |
| **D6** | §13.21 PR-6 | §3.19 integration: round_definition customization via §17.5 fork-and-edit + canon §17.5 alignment | D1, B1-B6 |
| **D7** | §13.21 PR-7 | 3 default round reports via §3.21 (per-template pass-rate, per-stop failure heatmap, per-assignee velocity) + acceptance demo | D6, E1-E3 |
| **E4** | §13.22 PR-4 | §28-aware aggregation safety: row-masking before + field-masking after + k-anonymity + per-recipient slicing | E1, canon §28 evaluator |
| **E5** | §13.22 PR-5 | `ReportExportProcessor` CSV/PDF/JSON generators + §3.12 evidence_artifact integration + delivery via §3.11 / NotificationService | E1, 079-083, 062-065 |
| **E6** | §13.22 PR-6 | `ReportAlertEvaluator` + threshold predicates + cooldown + multi-channel dispatch | E1 |
| **E7** | §13.22 PR-7 | custom_sql_override gating + AST safety check + permission registry update | E1 |
| **E8** | §13.22 PR-8 | Schema-change resilience: report materializations handle source schema changes per §3.19 yellow auto-migrate + canon §47 amendment | E3, C6 |
| **E9** | §13.22 PR-9 | Default report library: pack-installer ships default reports per pack (PM compliance %, MTTR, MTBF trend) | E1 |
| **E10** | §13.22 PR-10 | §3.20 rounds integration: ship 3 default round reports (per-template pass-rate, per-stop failure heatmap, per-assignee velocity) | D7, E9 |
...
| G0b | 53 (41 + 6 §3.19 B1-B6 + 3 §3.20 D1-D3 + 3 §3.21 E1-E3) | 0 (down from 11; §3.19+§3.20+§3.21 absorbed all 12) | No slip absorbed within gate; G0a slack (1 PR) is only safety net |
| G0c | 56 (38 + 7 §3.19 C1-C7 + 4 §3.20 D4-D7 + 7 §3.21 E4-E10) | 0 | None (tightest gate) — strongly recommend extending G0c to 7 weeks (push G1 from week 18 → week 19) given platform-capabilities-before-pack-work constraint |
...
8. 🔄 **§3.20 Generic Rounds embedded as G0b PRs D1-D3 + G0c PRs D4-D7** — 7 PRs total. Cross-pack rounds substrate (polymorphic stops + ad-hoc/scheduled + Glove-Mode + reporting integration) ready before G1 pack rounds (§14.1 maintenance-core rounds + clinical/facilities/ot-security rounds in their respective overlay packs).
9. 🔄 **§3.21 Platform Reporting Framework embedded as G0b PRs E1-E3 + G0c PRs E4-E10** — 10 PRs total. ReportBuilder + materialization + scheduled exports + alerting + §28-aware aggregation safety ready before G1 pack reports.
10. ✅ **Canon amendments listed in §8 of design spec** — each substrate worked example commits its canon amendment as part of its final PR (§13.7 PR-6 commits canon §32; §13.8 PR-8 commits canon §33; §13.20 PR-13 commits canon §45; §13.21 PR-5 commits canon §46; §13.22 PR-8 commits canon §47). Drafts already inline in the design spec's §24 amendment ledger.
