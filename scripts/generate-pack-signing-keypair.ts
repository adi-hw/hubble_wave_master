 ...
 *
 * Run via: npx tsx scripts/generate-pack-signing-keypair.ts
 *
 ...
 *   - PACK_SIGNING_PUBLIC_KEYS   (instance API, verification side — JSON map of key-id → PEM)
 */
function main(): void {
  ...
  const publicKeysJson = JSON.stringify({ default: publicPem.trim() });

  process.stdout.write('===== PACK_SIGNING_PUBLIC_KEYS (JSON map — set as PACK_SIGNING_PUBLIC_KEYS) =====\n');
  process.stdout.write(`${publicKeysJson}\n`);
